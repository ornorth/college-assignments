...
int main(int argc, char** argv)
{
  ...
  double start_time, end_time, local_time, g_time;
  ...
  g_time = 0;
  MPI_Reduce(&local_time, &g_time, 1, MPI_DOUBLE, MPI_SUM, 0, MPI_COMM_WORLD);
  if (my_rank == 0)
  {
      printf("Madhava-Leibniz took %e seconds on %u iterations\n", g_time, length);
  }
  ...
  if (my_rank == 0)
  {
      printf("Euler took %e seconds on %u iterations\n", g_time, length);
  }
