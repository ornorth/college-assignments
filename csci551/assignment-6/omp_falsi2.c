...
int main(void)
{
    int xitr, incs, itr, maxmitr, rc;
    double x0,x1,allerr, time_elapsed, lower, upper, interval;
    ...
    incs = (int)(upper-lower)/interval;
#pragma omp parallel for num_threads(4)
    for (xitr = 0; xitr < incs; xitr++)
    {
        x0 = (xitr*interval)+lower;
        x1 = x0 + interval;
        double x2, x3;
        itr = 0;
    ...
    while (tmp && tmp->next)
    {
        diff = tmp->val - tmp->next->val;
        if (diff < 0) diff *= -1.0;
        if (diff < allerr)
        {
            printf("DIFF: %6.4f, %6.4f\n", tmp->val, tmp->next->val);
        }
        if (tmp->val > tmp->next->val)
        {
            printf("OUT OF ORDER: %6.4f -> %6.4f", tmp->val, tmp->next->val);
        }
        tmp = tmp->next;
    }
    //end tmp
